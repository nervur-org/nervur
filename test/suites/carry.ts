...
export interface Made {
  ...
  /** Time passing, where the carry runs on a clock the test moves; a carry on the engine's timers needs none. */
...
export const carrySuite = (name: string, make: () => Promise<Made>) => {
  ...
  test(`${name}: an address where no domain vouches names none`, async (t) => {
    const { one, nowhere, close } = await make();
    t.after(close);
    assert.deepEqual(await one.vouched({ ward: WARD, at: [] }), []);
    assert.deepEqual(await one.vouched({ ward: WARD, at: [nowhere, 'no address at all'] }), []);
  });
  ...
  test(`${name}: a box waits for its reply as long as its own wait, then may have been heard`, async (t) => {
    const { one, two, listen, pass, close } = await make();
    t.after(close);
    let release!: () => void;
    ...
    const sent = one.send({ ward: WARD, at: two.at({}), box: new Uint8Array([1]), wait: 20 });
    await pass?.(20);
    assert.deepEqual(await sent, { reply: null, heard: true });
  });
