...
test('Its key rests in the secret store, and its memory in the native store', async (t) => {
  ...
  assert.ok(places.some((key) => /\/p\/[0-9a-f]{16}\//.test(key)), 'the house’s rows, in its view of the ground’s memory');
  assert.ok(places.some((key) => /\/p\/[0-9a-f]{32}$/.test(key)), 'the ground’s drawer');
